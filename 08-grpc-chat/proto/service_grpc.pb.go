...
type BroadcastClient interface {
	SignIn(ctx context.Context, in *Connect, opts ...grpc.CallOption) (Broadcast_SignInClient, error)
...
func (c *broadcastClient) SignIn(ctx context.Context, in *Connect, opts ...grpc.CallOption) (Broadcast_SignInClient, error) {
	stream, err := c.cc.NewStream(ctx, &Broadcast_ServiceDesc.Streams[0], "/proto.Broadcast/SignIn", opts...)
	if err != nil {
		return nil, err
	}
	x := &broadcastSignInClient{stream}
...
type Broadcast_SignInClient interface {
	Recv() (*Message, error)
	grpc.ClientStream
}

type broadcastSignInClient struct {
	grpc.ClientStream
}

func (x *broadcastSignInClient) Recv() (*Message, error) {
	m := new(Message)
...
type BroadcastServer interface {
	SignIn(*Connect, Broadcast_SignInServer) error
...
func (UnimplementedBroadcastServer) SignIn(*Connect, Broadcast_SignInServer) error {
...
func _Broadcast_SignIn_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(Connect)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(BroadcastServer).SignIn(m, &broadcastSignInServer{stream})
}

type Broadcast_SignInServer interface {
	Send(*Message) error
	grpc.ServerStream
}

type broadcastSignInServer struct {
	grpc.ServerStream
}

func (x *broadcastSignInServer) Send(m *Message) error {
...
var Broadcast_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		{
			StreamName:    "SignIn",
			Handler:       _Broadcast_SignIn_Handler,
			ServerStreams: true,
		},
	},
